...
export function createComment(req, res) {
  const { taskId, content, body } = req.body || {};
  const commentText = content || body; // קבל גם content וגם body
  if (!taskId || !commentText) return res.status(400).json({ error: 'taskId and content/body required' });
  const membership = db.prepare(`
    SELECT 1 FROM team_members tm
    JOIN projects p ON p.team_id = tm.team_id
    JOIN tasks t ON t.project_id = p.id
  ...
  const info = db.prepare('INSERT INTO comments (task_id, user_id, body) VALUES (?,?,?)').run(taskId, req.user.id, commentText);
